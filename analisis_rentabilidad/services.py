# -*- coding: utf-8 -*-
"""
Servicios del módulo de Análisis de Rentabilidad
Lógica de negocio para el análisis de rentabilidad (Control de Ingreso Real)
"""
...
def generar_todos_los_skus_optimizado(df_mes_actual, df_completo, fecha_inicio, fecha_fin, año, mes):
    """
    FUNCIÓN SÚPER OPTIMIZADA: Genera datos de TODOS los SKUs prioritarios en UNA SOLA operación
    ...
    Args:
        ...
        año: Año para filtrar clasificaciones
    ...
    print(f"🚀 SUPER OPTIMIZADO: Generando TODOS los SKUs en una sola operación")

    # 1. Obtener TODAS las clasificaciones en UNA SOLA consulta
    clasificaciones_agrupadas = obtener_todas_clasificaciones_optimizado(año, mes)

    # 2. ✅ NUEVO: Obtener datos de Ingreso Ideal (Silver.categorizacion_ingreso)
    ...
    print(f"⏱️  Carga Ingreso Ideal: {tiempo_ingreso_ideal_fin - tiempo_ingreso_ideal_inicio:.3f} segundos")

    # 2.5. ✅ OPTIMIZACIÓN CRÍTICA: Calcular variaciones UNA SOLA VEZ para TODOS los SKUs
    tiempo_variaciones_inicio = time.time()
    print(f"🔄 OPTIMIZACIÓN: Calculando variaciones para TODOS los SKUs en una sola operación...")
    ...
    # Expandir con gemelos para búsqueda histórica
    ...
    print(f"✅ OPTIMIZACIÓN: Variaciones calculadas en {tiempo_variaciones_fin - tiempo_variaciones_inicio:.3f} segundos")
    print(f"   📊 {len(todos_los_skus)} SKUs principales + {len(todos_los_skus_con_gemelos) - len(todos_los_skus)} gemelos")
    ...
    print(f"🔍 DEBUG: Clasificaciones recibidas de DB: {list(clasificaciones_agrupadas.keys())}")
    print(f"🔍 DEBUG: Mapeo disponible: {list(mapeo_resultado.keys())}")

    for clasificacion, skus_clasificacion in clasificaciones_agrupadas.items():
        print(f"🔍 DEBUG: Procesando clasificación: '{clasificacion}' con {len(skus_clasificacion) if skus_clasificacion else 0} SKUs")

        if not skus_clasificacion:
            print(f"⚠️  No hay SKUs {clasificacion}")
            continue

        resultado_key = mapeo_resultado.get(clasificacion)
        if not resultado_key:
            print(f"❌ DEBUG: No se encontró mapeo para '{clasificacion}'. Mapeo disponible: {list(mapeo_resultado.keys())}")
            continue

        print(f"📊 Procesando {len(skus_clasificacion)} SKUs {clasificacion}")

        # Extraer solo los SKUs para filtrar el DataFrame
        skus_list = [sku_data['sku'] for sku_data in skus_clasificacion]

        # ✅ MODIFICADO: Pasar datos de Ingreso Ideal Y variaciones pre-calculadas
    ...
    print(f"🎯 SUPER OPTIMIZADO COMPLETADO: {tiempo_total:.3f} segundos")
    print(f"📊 Resultados: Estrella({len(resultado['skus_estrella'])}), Prometedores({len(resultado['skus_prometedores'])}), Potenciales({len(resultado['skus_potenciales'])}), Revisión({len(resultado['skus_revision'])}), Remover({len(resultado['skus_remover'])})")

    return resultado

def calcular_resumen_rentabilidad(df_ventas, f1, f2, df_completo=None, mes_seleccionado=None):
    """
    ✅ OPTIMIZADO: Función específica para Control de Ingreso Real
    Calcula solo métricas de rentabilidad sin lógica de metas ni cumplimiento.

    Elimina operaciones innecesarias de calcular_cumplimiento_metas():
    - Cálculos de cumplimiento vs metas
    - Comparaciones con metas inexistentes
    - Construcción de cumplimiento_list que se descarta

    Args:
        df_ventas: DataFrame con datos de ventas del período
        f1, f2: Fechas de inicio y fin del período
        df_completo: DataFrame completo para comparaciones temporales (opcional)
        mes_seleccionado: Mes seleccionado para calcular evolución (opcional, por defecto mes actual)

    Returns:
        dict: resumen_general con métricas de rentabilidad
    """
    import sys
    ...
    print(f"✅ OPTIMIZADO: Calculando resumen de rentabilidad (sin metas)")
    ...
    # Filtrar solo datos válidos (excluir cancelados)
    ventas_periodo = df_ventas_naive[
        df_ventas_naive["estado"] != "Cancelado"
    ].copy()

    if ventas_periodo.empty:
        print("WARNING: No hay ventas válidas en el período")
        return {}

    print(f"✅ Ventas válidas en período: {len(ventas_periodo)} transacciones")

    # ✅ CÁLCULOS OPTIMIZADOS: Solo métricas necesarias usando campos pre-calculados
    ...
    # ✅ CÁLCULO INDIVIDUAL DE COMPONENTES DE GASTOS DIRECTOS
    ...
    # ✅ NUEVO: Calcular ROI usando la misma fórmula que la tabla para consistencia
    roi_promedio = (ingreso_real_periodo / costo_venta_periodo * 100) if costo_venta_periodo > 0 else 0.0

    print(f"✅ ROI promedio calculado: {roi_promedio:.2f}% (usando fórmula: Ingreso_Real / Costo_Venta * 100)")

    # Cálculos de promedios por transacción
    ...
    # ✅ COMPARACIÓN TEMPORAL OPTIMIZADA (si está disponible)
    ingreso_real_porcentaje_anterior = 0.0
    variacion_ingreso_pct = 0.0
    ventas_totales_anterior = 0.0
    variacion_ventas_pct = 0.0
    periodo_comparacion = 'Sin comparación'
    dias_comparados = 0

    if df_completo is not None and not df_completo.empty:
        try:
            # Lógica de comparación temporal simplificada
            ...
            primer_dia_anterior = datetime(año_anterior, mes_anterior, 1)
            ultimo_dia_anterior = datetime(año_anterior, mes_anterior, calendar.monthrange(año_anterior, mes_anterior)[1])
            ...
            if not ventas_mes_anterior.empty:
                ...
                periodo_comparacion = f"vs {calendar.month_name[mes_anterior]} {año_anterior}"
                dias_comparados = (ultimo_dia_anterior - primer_dia_anterior).days + 1

                print(f"✅ Comparación temporal: {periodo_comparacion}")
                print(f"✅ Variación ingreso: {variacion_ingreso_pct:+.1f}pp")

        except Exception as e:
            print(f"WARNING: Error en comparación temporal: {e}")
            # Continuar con valores por defecto

    # ✅ RESULTADO OPTIMIZADO: Solo campos necesarios
    resumen_general = {
        ...
        # Campos de comparación temporal
        ...
        # ❌ ELIMINADOS: Campos innecesarios de metas
        # 'meta_total': 0.0,
        # 'cumplimiento_global': 0.0,
        # 'cumplimiento_vs_meta_mes': 0.0,
    }

    # ===== CALCULAR EVOLUCIÓN DE 3 MESES PARA GRÁFICOS =====
    evolucion_costo = []
    evolucion_ventas = []
    evolucion_ingreso = []
    ...
    try:
        ...
        # 🎯 DETERMINAR FECHA DE REFERENCIA SEGÚN MES SELECCIONADO
        hoy_sistema = datetime.now()
        mes_actual_sistema = hoy_sistema.month
        año_actual_sistema = hoy_sistema.year
        dia_actual_sistema = hoy_sistema.day

        # Si no se especificó mes, usar el mes actual del sistema
        if mes_seleccionado is None:
            mes_seleccionado = mes_actual_sistema

        print(f"📅 Calculando evolución para mes seleccionado: {mes_seleccionado}")
        print(f"📅 Mes actual del sistema: {mes_actual_sistema}")

        # Determinar fecha de referencia y año
        # Si el mes seleccionado es mayor al actual, asumimos que es del año pasado
        if mes_seleccionado > mes_actual_sistema:
            año_referencia = año_actual_sistema - 1
        else:
            año_referencia = año_actual_sistema

        # Crear fecha de referencia del mes seleccionado
        fecha_referencia = pd.Timestamp(año_referencia, mes_seleccionado, 1)

        print(f"📅 Fecha de referencia: {fecha_referencia.strftime('%Y-%m')}")

        # 🎯 DETERMINAR SI ESTAMOS ANALIZANDO EL MES ACTUAL O UN MES PASADO
        es_mes_actual_seleccionado = (mes_seleccionado == mes_actual_sistema and año_referencia == año_actual_sistema)

        if es_mes_actual_seleccionado:
            print(f"🔍 Análisis del MES ACTUAL ({mes_seleccionado}) → Comparación justa (mismos días en los 3 meses)")
        else:
            print(f"🔍 Análisis de MES COMPLETO ({mes_seleccionado}) → Meses completos en los 3 meses")

        # Calcular para los últimos 3 meses desde el mes seleccionado
        for i in range(3):
            fecha_mes = fecha_referencia.replace(day=1) - pd.DateOffset(months=i)
            año_mes = fecha_mes.year
            mes_numero = fecha_mes.month
            mes_nombre = fecha_mes.strftime('%b')

            # 🎯 DETERMINAR HASTA QUÉ DÍA COMPARAR
            ultimo_dia_del_mes = calendar.monthrange(año_mes, mes_numero)[1]

            if es_mes_actual_seleccionado:
                # CASO 1: Filtro = MES ACTUAL → COMPARACIÓN JUSTA
                # Usar el mismo día en todos los meses para comparación justa
                dia_limite = min(dia_actual_sistema, ultimo_dia_del_mes)
                print(f"  → {mes_nombre} {año_mes}: usando día {dia_limite} (comparación justa con día actual)")
            else:
                # CASO 2: Filtro = MES PASADO → MESES COMPLETOS
                # Usar el mes completo para análisis histórico
                dia_limite = ultimo_dia_del_mes
                print(f"  → {mes_nombre} {año_mes}: usando día {dia_limite} (mes completo)")

            # Filtrar ventas del mes específico hasta el día límite
            # IMPORTANTE: Incluir TODO el día hasta las 23:59:59
            inicio_mes = pd.Timestamp(año_mes, mes_numero, 1)
            fin_rango = pd.Timestamp(año_mes, mes_numero, dia_limite, 23, 59, 59)

            # Usar df_completo si está disponible (para evolución 3 meses), sino cargar individual
            if df_completo is not None and not df_completo.empty:
                ...
                # Fallback: cargar datos específicos del mes (solo si no es el mes actual)
                if i == 0:  # Mes actual - usar datos ya cargados
                    ...
                    print(f"⚠️ Sin datos de {mes_nombre} - df_completo no disponible")
            ...
            if not ventas_mes.empty:
                ...
                # Usar campos pre-calculados si están disponibles, sino calcular
                ...
                # ✅ NUEVO: Calcular ROI del mes usando la misma fórmula que la tabla
            ...
            fecha_formateada = f"{mes_numero:02d}/{str(año_mes)[2:]}"  # Formato MM/YY
            ...
            # ✅ NUEVO: Añadir Ingreso Real nominal para gráfico de tarjeta nominal
            evolucion_ingreso_nominal.append({
                'fecha': fecha_formateada,
                'ingreso_real': float(ingreso_real_mes)
            })

            # ✅ NUEVO: Añadir ROI a la evolución
            evolucion_roi.append({
                'fecha': fecha_formateada,
                'roi': float(roi_mes)
            })

            print(f"📊 Evolución {mes_nombre} ({fecha_formateada}): Ventas=${total_ventas_mes:,.0f}, Costo={porcentaje_costo_mes:.1f}%, Ingreso={porcentaje_ingreso_mes:.1f}%, ROI={roi_mes:.1f}%")
        ...
        print(f"❌ Error calculando evolución 3 meses: {e}")
        evolucion_costo = []
        evolucion_ventas = []
        evolucion_ingreso = []
        evolucion_ingreso_nominal = []
    ...
    # Agregar campos de evolución al resumen
    ...
    print(f"✅ OPTIMIZADO: Resumen de rentabilidad calculado")
    print(f"✅ Ventas: ${ventas_totales_periodo:,.0f}")
    print(f"✅ Ingreso Real: ${ingreso_real_periodo:,.0f} ({ingreso_real_porcentaje:.1f}%)")
    print(f"✅ ROI Promedio: {roi_promedio:.1f}%")
    print(f"✅ Evolución costo: {len(evolucion_costo)} puntos")
    print(f"✅ Evolución ventas: {len(evolucion_ventas)} puntos")
    print(f"✅ Evolución ingreso: {len(evolucion_ingreso)} puntos")
    print(f"✅ Evolución ingreso nominal: {len(evolucion_ingreso_nominal)} puntos")
    print(f"✅ Evolución ROI: {len(evolucion_roi)} puntos")

    # ===== CALCULAR COMPARACIÓN CON MES ANTERIOR PARA ROI =====
    try:
        if df_completo is not None and not df_completo.empty:
            print("📊 Calculando comparación ROI con mes anterior...")
            ...
            # Filtrar datos del mes anterior
            inicio_mes_anterior = pd.Timestamp(año_anterior, mes_anterior, 1)
            if mes_anterior == 12:
                fin_mes_anterior = pd.Timestamp(año_anterior, 12, 31)
            else:
            ...
            if not ventas_mes_anterior.empty:
                ...
                print(f"✅ ROI Mes Anterior: {roi_anterior:.1f}%")
                print(f"✅ Variación ROI: {variacion_roi:+.1f}pp")
            else:
                print("⚠️  No hay datos del mes anterior para comparar ROI")
                resumen_general['roi_promedio_anterior'] = 0.0
                resumen_general['variacion_roi_pct'] = 0.0
        else:
            print("⚠️  No hay datos completos disponibles para comparar ROI")
            resumen_general['roi_promedio_anterior'] = 0.0
            resumen_general['variacion_roi_pct'] = 0.0

    except Exception as e:
        print(f"❌ Error calculando comparación ROI: {e}")
        resumen_general['roi_promedio_anterior'] = 0.0
        resumen_general['variacion_roi_pct'] = 0.0
...
def generar_datos_canales_principales(df, fecha_inicio, fecha_fin):
    """Genera datos de rentabilidad por canal principal usando la misma lógica que cumplimiento_metas"""
    import time
    import sys

    tiempo_funcion_inicio = time.time()
    print(f"🔍 [PERFORMANCE] INICIO - generar_datos_canales_principales()")
    sys.stdout.flush()

    if df.empty:
        return []

    print(f"INFO: Generando datos de canales principales con lógica de cumplimiento_metas")
    ...
    print(f"⏱️  [PYTHON] Normalización fechas: {tiempo_normalizacion_fin - tiempo_normalizacion_inicio:.3f} segundos")
    sys.stdout.flush()

    # Filtrar datos del período actual (excluir cancelados y filtrar por canales oficiales)
    ...
    print(f"⏱️  [PYTHON] Filtrado datos: {tiempo_filtrado_fin - tiempo_filtrado_inicio:.3f} segundos")
    sys.stdout.flush()

    if ventas_periodo.empty:
        print("No hay datos válidos para el período seleccionado")
        return []
    ...
    print(f"⏱️  [PYTHON] Agrupación por canal OPTIMIZADA: {tiempo_agrupacion_fin - tiempo_agrupacion_inicio:.3f} segundos")
    sys.stdout.flush()
    ...
    # ✅ OPTIMIZADO: Agrupar por canal y marca usando campos pre-calculados
    ...
    print(f"✅ OPTIMIZADO: Canal principal usando campos pre-calculados de ClickHouse")
    sys.stdout.flush()
    ...
    # ✅ NUEVO: Calcular ROI porcentaje usando métricas agregadas
    ventas_por_canal['ROI_Porcentaje'] = (
        ventas_por_canal['Ingreso_Real'] / ventas_por_canal['Costo_Venta'] * 100
    ).fillna(0)

    # ✅ OPTIMIZADO: Calcular promedios por unidad vendida (vectorizado)
    ...
    # ✅ OPTIMIZADO: Canal + Marca - usar campos pre-calculados
    ...
    # ✅ NUEVO: Calcular ROI porcentaje para Canal + Marca
    ...
    # ✅ OPTIMIZADO: Canal + Marca + Categoría - usar campos pre-calculados (ya no se calculan métricas derivadas)
    ...
    # ✅ OPTIMIZADO: Calcular PRECIO PROMEDIO POR UNIDAD por canal, marca y categoría (vectorizado)
    ...
    # Calcular PORCENTAJES POR CANAL, MARCA Y CATEGORÍA
    ...
    # ✅ NUEVO: Calcular ROI porcentaje para Canal + Marca + Categoría
    ...
    # ====== COMPARACIÓN CON MES ANTERIOR - LÓGICA EXACTA DE CUMPLIMIENTO_METAS ======
    print("=== CALCULANDO VARIACIÓN CON MES ANTERIOR ===")
    ...
    if mes_actual == hoy.month and año_actual == hoy.year:
        ...
        # Ajustar el día al último día válido del mes anterior si excede
        import calendar
        ultimo_dia_mes_anterior = calendar.monthrange(año_anterior, mes_anterior)[1]
        dia_ajustado = min(dia_actual, ultimo_dia_mes_anterior)

        # CORRECCIÓN: Establecer f2_anterior al final del día (23:59:59) para incluir todas las ventas de ese día
        f2_anterior = pd.Timestamp(año_anterior, mes_anterior, dia_ajustado, 23, 59, 59)
        ...
        f1_anterior = pd.Timestamp(año_anterior, mes_anterior, 1)
        if mes_anterior == 12:
            # CORRECCIÓN: Establecer al final del día 31 (23:59:59)
            f2_anterior = pd.Timestamp(año_anterior, 12, 31, 23, 59, 59)
        else:
            siguiente_mes = pd.Timestamp(año_anterior, mes_anterior + 1, 1)
            # CORRECCIÓN: Restar 1 segundo en lugar de 1 día para obtener el último segundo del mes
            f2_anterior = siguiente_mes - pd.Timedelta(seconds=1)
    ...
    try:
        ...
        print(f"⏱️  [PYTHON] Carga datos comparación: {tiempo_comparacion_fin - tiempo_comparacion_inicio:.3f} segundos")
        sys.stdout.flush()
        ...
        if not df_ventas_comparacion.empty:
            ...
            try:
                ...
                if not df_fallback.empty:
                    ...
                    if not ventas_periodo_anterior.empty:
                        # ✅ FALLBACK: Intentar usar campos pre-calculados si están disponibles
                        try:
                            ...
                            # Fallback a cálculo manual si no están los campos pre-calculados
                            ventas_por_canal_anterior = ventas_periodo_anterior.groupby('Channel').agg({
                                'Total': 'sum',
                                'Costo de venta': 'sum',
                        ...
                        # ✅ NUEVO: Calcular porcentajes faltantes para datos del mes anterior
                        ...
                        # ✅ FALLBACK: También calcular categorías con campos optimizados si están disponibles
                        try:
                            # Intentar primero con campos optimizados
                            ventas_por_canal_marca_categoria_anterior = ventas_periodo_anterior.groupby(['Channel', 'Marca', 'Categoria_Catalogo']).agg({
                                'Total': 'sum',                    # Ventas totales
                        ...
                        # ✅ NUEVO: Calcular ROI porcentaje para categorías mes anterior (fallback)
                        ...
                        print(f"✅ FALLBACK: {len(ventas_por_canal_marca_categoria_anterior)} categorías del mes anterior")
    ...
    # Agregar datos de comparación a los DataFrames principales - LÓGICA EXACTA DE CUMPLIMIENTO_METAS
    if not ventas_por_canal_anterior.empty and 'Canal' in ventas_por_canal_anterior.columns:
        # 🔍 DEBUG: Verificar qué columnas tiene ventas_por_canal_anterior
        print(f"🔍 DEBUG: Columnas en ventas_por_canal_anterior: {list(ventas_por_canal_anterior.columns)}")
        ...
        # Calcular variación NOMINAL del ingreso real (comparando montos en dinero)
        ...
        # Calcular variación en PUNTOS PORCENTUALES del % ingreso real
        ...
        # ✅ NUEVO: Calcular variación de ROI vs mes anterior (puntos porcentuales)
        ...
        print("✅ Variaciones calculadas exitosamente")
    else:
        ...
        ventas_por_canal['Periodo_Comparacion'] = "Sin datos del período anterior"
        ventas_por_canal['Dias_Comparados'] = 0
        print("⚠️  Sin datos para comparación temporal")
    ...
    CANALES_PRINCIPALES = [
        ...
        'Aliexpress', 'Coppel', 'TikTok Shop', 'Temu'
    ]
    ...
    for _, canal in ventas_por_canal.iterrows():
        ...
        if variacion_ingreso != 0:
            ...
            print(f"✓ {canal['Canal']}: {canal['Ingreso_Real_Porcentaje']:.1f}% "
    ...
    # ✅ ORDENAR CANALES POR VENTAS REALES (de mayor a menor)
    canales_ordenados_por_ventas = ventas_por_canal[ventas_por_canal['Canal'].isin(CANALES_PRINCIPALES)].sort_values('Ventas_Reales', ascending=False)['Canal'].tolist()
    print(f"✅ [ORDENAMIENTO] Canales ordenados por ventas: {canales_ordenados_por_ventas}")

    # Generar datos para cada canal principal - SIGUIENDO ESTRUCTURA DE CUMPLIMIENTO_METAS
    for canal_nombre in canales_ordenados_por_ventas:
        ...
        if not canal_row.empty:
            ...
            canal_data = {
                ...
                # CAMPOS DE COMPARACIÓN CON MES ANTERIOR
                ...
                'variacion_ingreso_pct': float(row['Variacion_Ingreso_Pct']) if 'Variacion_Ingreso_Pct' in row and pd.notna(row['Variacion_Ingreso_Pct']) else 0,  # Variación nominal del monto
                'variacion_ingreso_porcentaje_pct': float(row['Variacion_Ingreso_Porcentaje_Pct']) if 'Variacion_Ingreso_Porcentaje_Pct' in row and pd.notna(row['Variacion_Ingreso_Porcentaje_Pct']) else 0,  # Variación pp del %
                ...
                'periodo_comparacion': str(row['Periodo_Comparacion']) if 'Periodo_Comparacion' in row and pd.notna(row['Periodo_Comparacion']) else 'Sin comparación',
                'dias_comparados': int(row['Dias_Comparados']) if 'Dias_Comparados' in row and pd.notna(row['Dias_Comparados']) else 0
            }
        else:
            # Canal sin datos en el período
            canal_data = {
                'canal': canal_nombre,
                'es_fila_principal': True,
                'es_subfila': False,
        ...
        if not canal_row.empty:
            # Obtener datos del canal y marca anterior para las subfilas - CORREGIDO PARA INCLUIR TODAS LAS MÉTRICAS
            canal_anterior_marca = {}
            if not ventas_por_canal_marca_anterior.empty and 'Canal' in ventas_por_canal_marca_anterior.columns:
            ...
            if not loomber_row.empty:
                ...
                loomber_variacion_ingreso_nominal = 0  # Variación nominal del monto
                loomber_variacion_ingreso = 0  # Variación de puntos porcentuales
                ...
                if 'Loomber' in canal_anterior_marca:
                    ...
                    # Variación de PUNTOS PORCENTUALES (para columna % Ingreso)
                    loomber_variacion_ingreso = loomber['Ingreso_Real_Porcentaje'] - loomber_ingreso_anterior

                    # Variación NOMINAL del monto (para columna Ingreso Real)
                ...
                # Calcular representación dentro del canal
                ...
                loomber_data = {
                    ...
                    'variacion_ingreso_pct': float(loomber_variacion_ingreso_nominal),  # Variación nominal del monto
                    'variacion_ingreso_porcentaje_pct': float(loomber_variacion_ingreso),  # Variación pp del %
                    ...
                    'periodo_comparacion': str(canal_row.iloc[0]['Periodo_Comparacion']) if 'Periodo_Comparacion' in canal_row.iloc[0] else 'Sin comparación',
                ...
                # ✅ CORREGIDO: Generar sub-subfilas de categorías para Loomber JUSTO DESPUÉS de la subfila Loomber
                # Mostrar TODAS las categorías (no solo top 3)
                ...
                ].sort_values('Ventas_Reales', ascending=False)  # Ordenar por ventas (todas las categorías)

                for _, categoria_row in loomber_categorias.iterrows():
                    # NUEVO: Calcular variación con mes anterior para esta categoría específica
                    categoria_variacion_ventas = 0.0
                    categoria_variacion_ingreso = 0.0  # Variación en puntos porcentuales
                    categoria_variacion_ingreso_nominal = 0.0  # Variación nominal del monto
                    ...
                    # Buscar datos exactos de esta categoría en el mes anterior
                    if not ventas_por_canal_marca_categoria_anterior.empty and 'Canal' in ventas_por_canal_marca_categoria_anterior.columns:
                        try:
                            # Buscar datos exactos de esta categoría específica del mes anterior
                            ...
                            if not categoria_anterior_data.empty:
                                ...
                                # Calcular variación en PUNTOS PORCENTUALES (para columna % Ingreso)
                                categoria_variacion_ingreso = float(categoria_row['Ingreso_Real_Porcentaje']) - categoria_ingreso_anterior

                                # Calcular variación NOMINAL del monto (para columna Ingreso Real)
                                ...
                                # ✅ NUEVO: Calcular variación de ROI para categorías
                                ...
                                # ✅ NUEVO: Asegurar que las variables estén inicializadas cuando no hay datos anteriores
                                categoria_roi_anterior = 0.0
                                categoria_variacion_roi = float(categoria_row['ROI_Porcentaje']) - categoria_roi_anterior

                        except Exception as e:
                            print(f"Error calculando variación exacta para categoría Loomber {categoria_row['Categoria']}: {e}")
                    ...
                    categoria_data = {
                        ...
                        'variacion_ingreso_pct': float(categoria_variacion_ingreso_nominal),  # Variación nominal del monto
                        'variacion_ingreso_porcentaje_pct': float(categoria_variacion_ingreso),  # Variación pp del %
            ...
            if not otros_rows.empty:
                ...
                # ✅ NUEVO: Calcular ROI para Otros usando datos agregados
                ...
                otros_variacion_ingreso_nominal = 0  # Variación nominal del monto
                otros_variacion_ingreso = 0  # Variación de puntos porcentuales
                ...
                # Calcular representación dentro del canal para Otros
                ...
                otros_data = {
                    ...
                    'variacion_ingreso_pct': float(otros_variacion_ingreso_nominal),  # Variación nominal del monto
                    'variacion_ingreso_porcentaje_pct': float(otros_variacion_ingreso),  # Variación pp del %
                ...
                # ✅ CORREGIDO: Generar sub-subfilas de categorías para Otros JUSTO DESPUÉS de la subfila Otros
                # Mostrar TODAS las categorías (no solo top 3)
                ...
                for _, categoria_row in otros_categorias.iterrows():
                    # NUEVO: Calcular variación con mes anterior para esta categoría específica (Otros)
                    categoria_variacion_ventas = 0.0
                    categoria_variacion_ingreso = 0.0  # Variación en puntos porcentuales
                    categoria_variacion_ingreso_nominal = 0.0  # Variación nominal del monto
                    categoria_variacion_costo = 0.0
                    ...
                    # Buscar datos exactos de esta categoría en el mes anterior (para marcas Otros)
                    if not ventas_por_canal_marca_categoria_anterior.empty and 'Canal' in ventas_por_canal_marca_categoria_anterior.columns:
                        try:
                            # Buscar datos exactos de esta categoría específica para la marca "Otros" del mes anterior
                            categoria_anterior_data = ventas_por_canal_marca_categoria_anterior[
                                (ventas_por_canal_marca_categoria_anterior['Canal'] == canal_nombre) &
                            ...
                            if not categoria_anterior_data.empty:
                                # USAR DATOS EXACTOS (datos de la marca "Otros" de esta categoría)
                                ...
                                categoria_variacion_costo = float(categoria_row['Costo_Venta_Porcentaje']) - categoria_costo_anterior
                                categoria_variacion_gastos = float(categoria_row['Gastos_Directos_Porcentaje']) - categoria_gastos_anterior
                                # ✅ NUEVO: Calcular variación de ROI para categorías (Otros)
                            ...
                            print(f"Error calculando variación exacta para categoría Otros {categoria_row['Categoria']}: {e}")
    ...
    print(f"INFO: Generadas {len([c for c in canales_data if c.get('es_categoria_row', False)])} sub-subfilas de categoría")
    ...
    print(f"✓ Canales con datos de comparación: {canales_con_variacion}/{len([c for c in canales_data if c['es_fila_principal']])}")

    tiempo_funcion_fin = time.time()
    print(f"🔍 [PERFORMANCE] FIN - generar_datos_canales_principales(): {tiempo_funcion_fin - tiempo_funcion_inicio:.3f} segundos")
    sys.stdout.flush()
...
def unificar_productos_por_categoria_dominante(skus_estrella, skus_prometedores, skus_potenciales, skus_revision=None, skus_remover=None):
    """
    Unifica productos considerando SKUs componentes de todas las categorías.
    Determina la categoría donde mostrar cada producto unificado basándose en el SKU con mayores ventas.
    Aplica reclasificación a Estrella si el producto unificado supera el umbral de unidades vendidas.
    ...
    Args:
        ...
        skus_revision: Lista de SKUs Revisión (opcional)
    ...
    print(f"🔄 Iniciando unificación global de productos...")
    ...
    print(f"📊 Input: {len(skus_estrella)} Estrella, {len(skus_prometedores)} Prometedores, {len(skus_potenciales)} Potenciales, {len(skus_revision)} Revisión, {len(skus_remover)} Remover")

    # ✅ NUEVO: Calcular umbral dinámico para reclasificación a Estrella
    dia_actual_mes = datetime.now().day
    umbral_estrella_unidades = (500 * dia_actual_mes) / 30
    print(f"⭐ Umbral Estrella calculado: {umbral_estrella_unidades:.1f} unidades (día {dia_actual_mes} del mes)")

    # Crear diccionario global con todos los SKUs de todas las categorías
    todos_los_skus = {}

    # Añadir SKUs de cada categoría con su categoría original
    ...
    print(f"📦 Total SKUs encontrados: {len(todos_los_skus)}")
    ...
    # Determinar categoría dominante para cada producto unificado
    ...
    for sku_principal, gemelos in PRODUCTOS_UNIFICADOS.items():
        ...
        if componentes_encontrados:
            ...
            # ✅ NUEVO: Calcular unidades totales del producto unificado
            unidades_totales = sum(comp.get('cantidad_total', 0) for comp in componentes_encontrados)

            # ✅ NUEVO: Reclasificar a Estrella si supera el umbral de unidades
            categoria_original = categoria_dominante
            if unidades_totales >= umbral_estrella_unidades and categoria_dominante != 'estrella':
                categoria_dominante = 'estrella'
                print(f"   🌟 RECLASIFICACIÓN: {sku_principal} de '{categoria_original}' → 'estrella' ({unidades_totales:.0f} unidades >= {umbral_estrella_unidades:.1f} umbral)")
            ...
            print(f"🎯 {sku_principal}: {len(componentes_encontrados)} componentes, {unidades_totales:.0f} unidades → Categoría: {categoria_dominante} (${componente_dominante['ventas_reales']:,.0f})")

    # Separar filas de totales por categoría
    ...
    # Aplicar unificación a cada categoría
    ...
    print(f"✅ Output: {len(skus_estrella_unificados)} Estrella, {len(skus_prometedores_unificados)} Prometedores, {len(skus_potenciales_unificados)} Potenciales, {len(skus_revision_unificados)} Revisión, {len(skus_remover_unificados)} Remover")

    # ✅ NUEVO: Mostrar resumen de reclasificaciones
    reclasificados = [info for info in productos_unificados_info.values()
                      if info.get('categoria_original') != info.get('categoria_dominante')]
    if reclasificados:
        print(f"🌟 RESUMEN RECLASIFICACIONES: {len(reclasificados)} producto(s) unificado(s) reclasificado(s) a Estrella")
        for info in reclasificados:
            sku_principal = [k for k, v in productos_unificados_info.items() if v == info][0]
            print(f"   • {sku_principal}: {info['unidades_totales']:.0f} unidades (umbral: {umbral_estrella_unidades:.1f})")

    # DEBUG: Verificar posición de fila de totales en Prometedores
    if skus_prometedores_unificados:
        primer_item = skus_prometedores_unificados[0]
        ultimo_item = skus_prometedores_unificados[-1]
        print(f"🔍 DEBUG PROMETEDORES - Primer item: es_fila_total={primer_item.get('es_fila_total', False)}, producto={primer_item.get('producto', primer_item.get('sku', 'N/A'))[:30]}")
        print(f"🔍 DEBUG PROMETEDORES - Último item: es_fila_total={ultimo_item.get('es_fila_total', False)}, producto={ultimo_item.get('producto', ultimo_item.get('sku', 'N/A'))[:30]}")
...
def unificar_skus_para_categoria(categoria_objetivo, productos_unificados_info, todos_los_skus, filas_totales):
    """
    Unifica SKUs para una categoría específica basándose en la información de productos unificados.
    """
    print(f"🔧 Procesando categoría: {categoria_objetivo}")

    resultado_final = []
    skus_ya_procesados = set()

    # Crear diccionario inverso para saber qué SKUs pertenecen a productos unificados
    mapeo_inverso = {}
    for sku_principal, gemelos in PRODUCTOS_UNIFICADOS.items():
        mapeo_inverso[sku_principal] = sku_principal
        for gemelo in gemelos:
    ...
    # 1. Procesar productos unificados que pertenecen a esta categoría
    productos_unificados_de_categoria = []
    skus_no_unificados = []  # Inicializar aquí

    for sku_principal, info in productos_unificados_info.items():
        if info['categoria_dominante'] == categoria_objetivo:
            print(f"   📦 Procesando producto unificado: {sku_principal} con {len(info['componentes'])} componentes")
            ...
            if producto_unificado:
                # Verificar si se unificó o si es un SKU individual
                if producto_unificado.get('es_producto_unificado', False):
                    ...
                    for componente in info['componentes']:
                        ...
                        print(f"      ✓ Componente procesado: {componente['sku']}")
                else:
                    # Es un SKU individual (solo 1 componente con ventas)
                    # Se tratará como SKU no unificado
                    ...
                    print(f"      ✓ SKU individual (no unificado): {producto_unificado['sku']}")

    # 2. Añadir otros SKUs no unificados de esta categoría
    ...
    for item in items_para_ordenar:
        if item.get('es_producto_unificado', False):
            # Es un producto unificado, añadir el producto + sus componentes
            resultado_final.append(item)

            # Buscar los componentes de este producto específico
            ...
            for grupo in productos_unificados_de_categoria:
                if grupo['sku_principal'] == sku_principal:
                    ...
                    # Añadir componentes individuales para expansión
            ...
            # Es un SKU no unificado, añadir tal como está
            resultado_final.append(item)

    # 6. Añadir totales al final
    resultado_final.extend(filas_totales)

    print(f"   ✅ Resultado final para {categoria_objetivo}: {len(resultado_final)} items")
    return resultado_final


def crear_producto_unificado(sku_principal, componentes):
    """
    Crea un producto unificado sumando las métricas de todos sus componentes.
    SOLO unifica si hay 2 o más componentes con ventas.
    ...
    print(f"   🔍 Componentes totales: {len(componentes)}, con ventas: {len(componentes_con_ventas)}")

    # Si solo hay 1 componente con ventas, NO unificar
    if len(componentes_con_ventas) == 1:
        print(f"   ⚠️ Solo 1 componente con ventas → NO se unifica, se trata como SKU individual")
    ...
    if len(componentes_con_ventas) == 0:
        print(f"   ⚠️ Ningún componente con ventas → No se crea producto unificado")
        return None

    # Si hay 2 o más componentes con ventas, SÍ unificar
    print(f"   ✅ {len(componentes_con_ventas)} componentes con ventas → SÍ se unifica")

    # Usar el primer componente como base
    producto_base = componentes_con_ventas[0].copy()

    # Sumar métricas financieras SOLO de componentes con ventas (mes actual)
    ...
    print(f"   📊 Buscando datos históricos de {len(skus_todos_componentes)} componentes totales para calcular variaciones")

    # Buscar datos históricos de TODOS los componentes
    ...
    for componente in componentes_con_ventas:
        # Sumar datos históricos de componentes que SÍ vendieron
        ...
        print(f"      📊 Componente {sku_comp}: Sept ${componente['ventas_reales']:,.0f}, Ago ${ventas_ant:,.0f}, Costo Ago {(costo_ant/ventas_ant*100) if ventas_ant > 0 else 0:.1f}%")
    ...
    producto_unificado = {
        ...
        'es_fila_total': False,
        # Campos de variación temporal - valores absolutos
    ...
    producto_unificado['variacion_ingreso_porcentaje_pct'] = ingreso_porcentaje_actual - ingreso_porcentaje_anterior  # CORREGIDO: nombre de campo
    producto_unificado['variacion_roi_pct'] = roi_porcentaje_actual - roi_porcentaje_anterior

    # Calcular variación nominal del monto de ingreso real (porcentaje de cambio)
    producto_unificado['variacion_ingreso_pct'] = ((ingreso_real_total - ingreso_anterior_total) / abs(ingreso_anterior_total) * 100) if ingreso_anterior_total != 0 else 0.0
...
def consolidar_desglose_canales(componentes):
    ...
    print(f"🔗 Consolidando desgloses por canal para {len(componentes)} componentes...")
...
def consolidar_desglose_meses(componentes):
    ...
    print(f"📅 Consolidando desgloses por mes para {len(componentes)} componentes...")
    meses_consolidados = {}
        ...
        for mes_data in desglose_meses:
            ...
            if mes not in meses_consolidados:
                meses_consolidados[mes] = {
                    ...
                    'periodo': mes_data.get('periodo', ''), 'año': mes_data.get('año', 0),
...
def obtener_todas_clasificaciones_optimizado(año=None, mes=None):
    """
    Función SÚPER OPTIMIZADA: Obtiene todas las clasificaciones en UNA SOLA consulta
    ...
    Returns:
        Diccionario con SKUs agrupados por clasificación:
        {
    ...
    try:
        tiempo_inicio = time.time()
        print(f"=== CONSULTA ÚNICA OPTIMIZADA ClickHouse PARA {mes}/{año} ===")
        print(f"🎯 Obteniendo TOP 100 SKUs de todas las clasificaciones prioritarias")
        ...
        print(f"⏱️  Query ClickHouse ÚNICA ejecutada en: {tiempo_query - tiempo_inicio:.3f} segundos")
        print(f"📊 Total SKUs obtenidos: {len(rows)}")

        # DEBUG: Mostrar qué clasificaciones llegan de la DB
        clasificaciones_db = set()
        for row in rows:
            clasificaciones_db.add(row[7])  # clasificacion
        print(f"🔍 DEBUG: Clasificaciones encontradas en DB: {sorted(list(clasificaciones_db))}")
        ...
        for row in rows:
            ...
            #     print(f"🔍 DEBUG: Mapeando '{clasificacion_clickhouse}' → '{clasificacion_python}'")

            # Solo procesar clasificaciones prioritarias
            if clasificacion_python not in clasificaciones_agrupadas:
                print(f"⚠️ DEBUG: Clasificación '{clasificacion_python}' no encontrada en agrupadas: {list(clasificaciones_agrupadas.keys())}")
                continue
            ...
            # Agregar a la clasificación correspondiente
            clasificaciones_agrupadas[clasificacion_python].append(sku_data)

        tiempo_final = time.time()
        print(f"⏱️  Procesamiento total OPTIMIZADO: {tiempo_final - tiempo_inicio:.3f} segundos")
        ...
        # Fallback: retornar diccionario vacío
        return {
            'Estrella': [],
            'Prometedores': [],
            'Potenciales': []
...
def obtener_ingreso_ideal_sku_canal():
    ...
    - Diferenciador: UInt8 (Ajuste por diferenciación en puntos positivos)
    - Calidad: UInt8 (Ajuste por calidad en puntos positivos)
    - Percepcion: UInt8 (Ajuste por percepción en puntos positivos)
    ...
    try:
        tiempo_inicio = time.time()
        print(f"📊 Consultando Silver.categorizacion_ingreso para Ingreso Ideal...")
        ...
        tiempo_query = time.time()
        print(f"⏱️  Query categorizacion_ingreso ejecutada en: {tiempo_query - tiempo_inicio:.3f} segundos")
        print(f"📊 Total registros obtenidos: {len(rows)}")
        ...
        tiempo_final = time.time()
        print(f"⏱️  Procesamiento Ingreso Ideal completado en: {tiempo_final - tiempo_inicio:.3f} segundos")
        print(f"✅ Ingreso Ideal cargado: {len(skus_unicos)} SKUs únicos, {len(canales_unicos)} canales únicos")
        print(f"✅ Total combinaciones SKU×Canal: {sum(len(v) for v in datos_ideal.values())}")

        return datos_ideal

    except Exception as e:
        print(f"❌ Error consultando Silver.categorizacion_ingreso: {e}")
        import traceback
        traceback.print_exc()
...
def resumen_clasificaciones_con_participacion(clasificaciones_actual):
    ...
    # Colores para cada clasificación - Paleta mejorada
    colores = {
        "Estrella": "#D4AF37", "Prometedores": "#28a745", "Potenciales": "#17a2b8",
        "Revisión": "#fd7e14", "Remover": "#e63946"
    }
...
# FUNCIONES ESPECÍFICAS PARA PESTAÑA DE RENTABILIDAD
def cargar_acumulado_mensual_matriz():
    """
    Carga datos de la vista Silver.RPT_Ventas_Acumulado_Mensual_SKU_Canal
    Vista con datos acumulados día a día dentro de cada mes por SKU y Canal
    """
    from datetime import datetime
    from dateutil.relativedelta import relativedelta
    import time
    ...
    tiempo_funcion_inicio = time.time()
    print(f"🔍 [PERFORMANCE] INICIO - cargar_acumulado_mensual_matriz()")
    sys.stdout.flush()

    tiempo_conexion_inicio = time.time()
    client = get_db_connection()
    tiempo_conexion_fin = time.time()
    print(f"⏱️  [PERFORMANCE] Conexión BD: {tiempo_conexion_fin - tiempo_conexion_inicio:.3f} segundos")
    sys.stdout.flush()
    ...
    try:
        ...
        # Query para cargar la vista completa (o filtrar por período si es necesario)
        ...
        print(f"⏱️  [PERFORMANCE] Ejecución query BD: {tiempo_ejecucion_fin - tiempo_ejecucion_inicio:.3f} segundos")
        sys.stdout.flush()
        ...
        print(f"⏱️  [PERFORMANCE] Conversión a DataFrame: {tiempo_dataframe_fin - tiempo_dataframe_inicio:.3f} segundos")
        sys.stdout.flush()
        ...
        print(f"⏱️  [PERFORMANCE] Generación listas: {tiempo_listas_fin - tiempo_listas_inicio:.3f} segundos")
        sys.stdout.flush()

        tiempo_query_fin = time.time()
        print(f"⏱️  [PERFORMANCE] Query TOTAL: {tiempo_query_fin - tiempo_query_inicio:.3f} segundos")

        tiempo_funcion_fin = time.time()
        print(f"🔍 [PERFORMANCE] FIN - cargar_acumulado_mensual_matriz(): {tiempo_funcion_fin - tiempo_funcion_inicio:.3f} segundos")
        sys.stdout.flush()
        ...
        if 'Clasificacion' in df.columns:
            ...
            print(f"INFO: [MATRIZ] Registros con clasificación: {df['Clasificacion'].notna().sum()}")
        else:
...
def cargar_ultimos_3_meses_rentabilidad():
    ...
    Carga datos optimizados de los últimos 3 meses para desglose temporal
    ...
    tiempo_funcion_inicio = time.time()
    print(f"🔍 [PERFORMANCE] INICIO - cargar_ultimos_3_meses_rentabilidad()")
    sys.stdout.flush()
    ...
    try:
        hoy = datetime.now()
        print(f"INFO: [RENTABILIDAD] Cargando datos de últimos 3 meses desde {hoy.strftime('%Y-%m-%d')}")
        ...
        # Query optimizada para 3 meses específicos + canales oficiales
        tiempo_query_inicio = time.time()
        query = f"""
...
def calcular_variaciones_temporales_skus(df_completo, skus_list):
    """
    Calcula las variaciones temporales para una lista de SKUs siguiendo la misma lógica que canales
    Args:
    ...
    print(f"🔍 Calculando variaciones temporales para {len(skus_list)} SKUs...")
    ...
    # Determinar el período de comparación basado en días transcurridos (igual que canales)
    hoy = date.today()
    fecha_actual = datetime.now()
    mes_actual = fecha_actual.month
    año_actual = fecha_actual.year
    ...
    if mes_actual == hoy.month and año_actual == hoy.year:
        ...
        f1_anterior = datetime(año_anterior, mes_anterior, 1)

        # Ajustar el día al último día válido del mes anterior si excede
        import calendar
        ultimo_dia_mes_anterior = calendar.monthrange(año_anterior, mes_anterior)[1]
        dia_ajustado = min(dias_transcurridos, ultimo_dia_mes_anterior)

        # CORRECCIÓN: Establecer f2_anterior al final del día (23:59:59) para incluir todas las ventas de ese día
        f2_anterior = datetime(año_anterior, mes_anterior, dia_ajustado, 23, 59, 59)

        periodo_comparacion = f"vs mismos {dias_transcurridos} días de {f1_anterior.strftime('%B %Y')}"
        print(f"COMPARACIÓN SKUs: {dias_transcurridos} días transcurridos")

    else:
        # Mes completo - usar todo el mes anterior
        ultimo_dia_mes = calendar.monthrange(mes_actual, año_actual)[1]
        ...
        f1_anterior = datetime(año_anterior, mes_anterior, 1)
        # CORRECCIÓN: Establecer al final del último día del mes (23:59:59)
        ultimo_dia = calendar.monthrange(año_anterior, mes_anterior)[1]
        f2_anterior = datetime(año_anterior, mes_anterior, ultimo_dia, 23, 59, 59)

        periodo_comparacion = f"vs {f1_anterior.strftime('%B %Y')}"
        print(f"COMPARACIÓN SKUs: mes completo")

    # Filtrar datos del período anterior para los SKUs especificados
    try:
        ...
        print(f"✅ VARIACIONES SKUs: {len(ventas_periodo_anterior)} registros del período anterior obtenidos")

        if not ventas_periodo_anterior.empty:
            # Calcular métricas por SKU del mes anterior usando campos pre-calculados
            ...
            # Calcular porcentajes ponderados para el período anterior
            ...
            # ✅ NUEVO: Calcular ROI porcentaje para el período anterior
            ...
            # También calcular variaciones por canal para cada SKU (para desglose por canal)
            ...
            # ✅ NUEVO: Calcular ROI porcentaje para canales del período anterior
            ...
            print(f"✅ Variaciones calculadas para {len(skus_anteriores)} SKUs con datos anteriores")
            return skus_anteriores
        else:
            print("⚠️ Sin datos del período anterior para SKUs")
            return {}

    except Exception as e:
        print(f"❌ Error calculando variaciones temporales para SKUs: {e}")
        return {}
...
def generar_datos_skus_genericos_optimizado(df_mes_actual, df_completo, fecha_inicio, fecha_fin, skus_list, clasificacion, datos_ingreso_ideal=None, skus_anteriores_precalculados=None):
    """
    Función genérica optimizada para procesar cualquier lista de SKUs
    ...
    Args:
        ...
        clasificacion: Nombre de la clasificación (para logs)
    ...
    # Si no se pasaron datos de ingreso ideal, usar diccionario vacío
    if datos_ingreso_ideal is None:
        datos_ingreso_ideal = {}

    # ✅ OPTIMIZADO: Normalizar fechas del DataFrame principal (sin copy innecesario)
    df_ventas_naive = df_mes_actual
    if 'Fecha' in df_ventas_naive.columns:
        df_ventas_naive = df_ventas_naive.copy()  # Solo copiar si necesitamos modificar
        df_ventas_naive["Fecha"] = pd.to_datetime(df_ventas_naive["Fecha"]).dt.tz_localize(None)

    # Filtrar datos del período actual (excluir cancelados, filtrar por canales oficiales y solo SKUs de la lista)
    ventas_periodo = df_ventas_naive[
        (df_ventas_naive["estado"] != "Cancelado") &
        (df_ventas_naive['Channel'].isin(CANALES_CLASIFICACION)) &
    ...
    if ventas_periodo.empty:
        print(f"⚠️  No hay datos válidos para SKUs {clasificacion} en el período seleccionado")
        return []

    print(f"✅ Ventas de SKUs {clasificacion} en período: {len(ventas_periodo)} registros")
    if datos_ingreso_ideal:
        print(f"✅ Datos de Ingreso Ideal disponibles para integración")

    # ✅ OPTIMIZADO: Usar campos pre-calculados de ClickHouse para montos, calcular porcentajes ponderados
    ventas_por_sku = ventas_periodo.groupby(['sku', 'descripcion']).agg({
        'Total': 'sum',                    # Ventas totales
        'Costo de venta': 'sum',          # Costo de venta (ya con IVA incluido)
        'Gastos_directos': 'sum',         # ¡YA CALCULADO EN CLICKHOUSE!
        'Ingreso real': 'sum',            # ¡YA CALCULADO EN CLICKHOUSE!
        'cantidad': 'sum',
        'estado': 'count'
    }).reset_index()

    # ✅ Calcular porcentajes ponderados correctamente para consistencia
    ...
    # ✅ NUEVO: Calcular ROI porcentaje para SKUs (igual metodología que canales)
    ...
    print(f"✅ OPTIMIZADO: Usando campos pre-calculados de ClickHouse para {clasificacion}")
    print(f"   • Gastos Directos: ya calculados")
    print(f"   • Ingreso Real: ya calculado")
    print(f"   • Porcentajes: calculados con método ponderado para consistencia")

    # Asegurar que Cantidad_Total sea numérica
    ventas_por_sku['Cantidad_Total'] = pd.to_numeric(ventas_por_sku['Cantidad_Total'], errors='coerce').fillna(0)

    # ✅ OPTIMIZADO: Calcular precio promedio por unidad vendida (vectorizado)
    ...
    # ✅ OPTIMIZADO: Agrupar ventas por SKU Y CANAL usando campos pre-calculados
    ...
    # ✅ Calcular porcentajes ponderados para canales (consistencia con totales)
    ...
    # ✅ NUEVO: Calcular ROI porcentaje para desglose por canal de SKUs
    ...
    print(f"✅ OPTIMIZADO: Desglose por canal usando campos pre-calculados")

    # Asegurar que Cantidad_Total sea numérica para el desglose por canal
    ventas_por_sku_canal['Cantidad_Total'] = pd.to_numeric(ventas_por_sku_canal['Cantidad_Total'], errors='coerce').fillna(0)

    # ✅ OPTIMIZADO: Calcular precio promedio por unidad vendida para el desglose por canal (vectorizado)
    ...
    # ✅ OPTIMIZACIÓN: Usar variaciones pre-calculadas si están disponibles
    if skus_anteriores_precalculados is not None:
        print(f"✅ OPTIMIZACIÓN: Usando variaciones pre-calculadas para {clasificacion}")
        ...
        print(f"⚠️ FALLBACK: Calculando variaciones para {clasificacion} (no se pasaron pre-calculadas)")
        skus_list_para_historia = skus_list.copy()
        for sku in skus_list:
            if sku in PRODUCTOS_UNIFICADOS:
                gemelos = PRODUCTOS_UNIFICADOS[sku]
                skus_list_para_historia.extend(gemelos)
                print(f"   🔗 Buscando datos históricos del producto unificado {sku} y sus gemelos: {gemelos}")
    ...
    # ✅ OPTIMIZACIÓN CRÍTICA: Pre-calcular desglose temporal de TODOS los SKUs UNA SOLA VEZ
    # Esto evita 80+ llamadas individuales a calcular_desglose_temporal_sku()
    import time
    tiempo_desglose_inicio = time.time()
    print(f"🚀 [OPTIMIZACIÓN CRÍTICA] Pre-calculando desglose temporal de {len(skus_list)} SKUs en UNA operación...")
    desgloses_temporales = calcular_desglose_temporal_todos_skus_optimizado(df_completo, skus_list)
    tiempo_desglose_fin = time.time()
    print(f"✅ [OPTIMIZACIÓN] Desgloses temporales pre-calculados en {tiempo_desglose_fin - tiempo_desglose_inicio:.3f} segundos")
    ...
    for _, row in ventas_por_sku.iterrows():
        ...
        # ✅ NUEVO: Obtener datos de variaciones temporales para este SKU
        sku_anterior = skus_anteriores.get(sku_actual, {})
        sku_variacion_ventas_pct = 0.0
        sku_variacion_ingreso_pct = 0.0  # Variación nominal del monto de ingreso real
        sku_variacion_ingreso_porcentaje_pct = 0.0  # Variación de puntos porcentuales del % ingreso
        sku_variacion_costo_pct = 0.0
        sku_variacion_gastos_pct = 0.0
        sku_variacion_roi_pct = 0.0
        sku_periodo_comparacion = 'Sin comparación'
        sku_dias_comparados = 0

        if sku_anterior:
            # Calcular variación de ventas del SKU (porcentaje de cambio relativo)
            ...
            # Calcular variación NOMINAL de ingreso real del SKU (comparando montos)
        ...
        for _, canal_row in canales_sku.iterrows():
            ...
            # ✅ NUEVO: Buscar datos de variaciones del canal anterior para este SKU específico
            canal_anterior = None
            canal_variacion_ventas_pct = 0.0
            canal_variacion_ingreso_pct = 0.0  # Variación nominal del monto de ingreso real
            canal_variacion_ingreso_porcentaje_pct = 0.0  # Variación de puntos porcentuales del % ingreso
            ...
            if sku_actual in skus_anteriores and 'canales_anteriores' in skus_anteriores[sku_actual]:
                if canal_actual in skus_anteriores[sku_actual]['canales_anteriores']:
                    ...
                    # Calcular variación de ventas del canal (porcentaje de cambio relativo)
                    ...
                    # Calcular variación NOMINAL de ingreso real del canal (comparando montos)
            ...
            if sku_actual in datos_ingreso_ideal and canal_actual in datos_ingreso_ideal[sku_actual]:
                ...
                if gap_ingreso_pp < 0:
                    # Potencial = Ventas × (IR_Ideal% - IR_Actual%) / 100
            ...
            canal_data = {
                ...
                # ✅ NUEVO: Campos de variaciones temporales para canal
                ...
                'variacion_ingreso_pct': float(canal_variacion_ingreso_pct),  # Variación nominal del monto
                'variacion_ingreso_porcentaje_pct': float(canal_variacion_ingreso_porcentaje_pct),  # Variación pp del %
        ...
        # ✅ OPTIMIZACIÓN CRÍTICA: Usar desglose temporal pre-calculado (en lugar de calcular 80+ veces)
        # ANTES: desglose_temporal = calcular_desglose_temporal_sku(df_completo, sku_actual)  # ← 80 copias del DataFrame!
        # AHORA: Búsqueda O(1) en diccionario pre-calculado
        desglose_temporal = desgloses_temporales.get(sku_actual, [])
        ...
        sku_item = {
            ...
            # ✅ NUEVO: Campos de variaciones temporales para SKU principal
            ...
            'variacion_ingreso_pct': float(sku_variacion_ingreso_pct),  # Variación nominal del monto
            'variacion_ingreso_porcentaje_pct': float(sku_variacion_ingreso_porcentaje_pct),  # Variación pp del %
    ...
    # CALCULAR FILA DE TOTALES para la clasificación
    if skus_data:
        print(f"📊 Calculando totales para SKUs {clasificacion}...")
        ...
        # ✅ NUEVO: Calcular ROI total (igual metodología que SKUs individuales)
        ...
        print(f"✅ Fila de totales agregada para SKUs {clasificacion}")
        print(f"📈 Total Ventas: ${total_ventas_reales:,.0f} | Ingreso Real: ${total_ingreso_real:,.0f} ({total_ingreso_real_porcentaje:.1f}%)")

    print(f"✅ Datos de SKUs {clasificacion} generados exitosamente: {len(skus_data)} SKUs (incluyendo totales)")
    return skus_data

def generar_datos_skus_estrella_rentabilidad(df_mes_actual, df_completo, fecha_inicio, fecha_fin):
    """
    SOLO PARA ANÁLISIS DE RENTABILIDAD
    Genera datos de SKUs Estrella con métricas del mes actual + desglose temporal de 3 meses
    """
    # Usar la función original pero con datos del mes actual para métricas principales
...
def generar_datos_skus_prometedores_rentabilidad(df_mes_actual, df_completo, fecha_inicio, fecha_fin):
    """
    SOLO PARA ANÁLISIS DE RENTABILIDAD
    Genera datos de SKUs Prometedores con métricas del mes actual + desglose temporal de 3 meses
    """
    # Usar la función original pero con datos del mes actual para métricas principales
...
def generar_datos_skus_potenciales_rentabilidad(df_mes_actual, df_completo, fecha_inicio, fecha_fin):
    """
    SOLO PARA ANÁLISIS DE RENTABILIDAD
    Genera datos de SKUs Potenciales con métricas del mes actual + desglose temporal de 3 meses
    """
    # Usar la función original pero con datos del mes actual para métricas principales
...
def calcular_desglose_temporal_todos_skus_optimizado(df_completo, skus_list):
    """
    ✅ OPTIMIZACIÓN CRÍTICA: Calcula el desglose temporal de TODOS los SKUs en UNA SOLA operación

    Esta función reemplaza las 80+ llamadas individuales a calcular_desglose_temporal_sku()
    que hacían 80 copias del DataFrame completo (42,379 registros × 80 = 3.4M operaciones).

    GANANCIA ESTIMADA: 20-25 segundos (de ~28s → ~3-8s)

    Args:
        df_completo: DataFrame completo con datos de 3 meses
        skus_list: Lista de SKUs a procesar

    Returns:
        Dict indexado por SKU con la misma estructura que calcular_desglose_temporal_sku():
        {
            'SKU123': [
                {
                    'mes': 10,
                    'año': 2025,
                    'periodo': '1-21 Oct 2025',
                    'ventas_reales': 12345.67,
                    'costo_venta': 5678.90,
                    'gastos_directos': 1234.56,
                    'ingreso_real': 5432.21,
                    'costo_venta_porcentaje': 46.0,
                    'gastos_directos_porcentaje': 10.0,
                    'ingreso_real_porcentaje': 44.0,
                    'roi_porcentaje': 95.7,
                    'num_transacciones': 15,
                    'ventas_reales_promedio': 823.04,
                    'costo_venta_promedio': 378.59,
                    'gastos_directos_promedio': 82.30,
                    'ingreso_real_promedio': 362.15,
                    'desglose_canales': [...]
                },
                ... (hasta 3 meses)
            ],
    ...
    tiempo_inicio = time.time()
    print(f"🚀 [OPTIMIZACIÓN] Calculando desglose temporal para {len(skus_list)} SKUs en UNA operación...")

    if df_completo.empty or not skus_list:
        return {}

    # ✅ PASO 1: Normalizar fechas UNA SOLA VEZ (evitar 80 copias)
    ...
    # ✅ PASO 2: Filtrar UNA SOLA VEZ para todos los SKUs
    df_filtrado = df_temporal[
        (df_temporal['sku'].isin(skus_list)) &
        (df_temporal["estado"] != "Cancelado") &
        (df_temporal['Channel'].isin(CANALES_CLASIFICACION))
    ].copy()

    if df_filtrado.empty:
        print(f"⚠️  No hay datos válidos para los SKUs especificados")
        return {}

    # ✅ PASO 3: Agregar columnas auxiliares para agrupación
    hoy = datetime.now()
    dia_actual = hoy.day

    # Agregar columnas de año, mes y día
    df_filtrado['año'] = pd.to_datetime(df_filtrado['Fecha']).dt.year
    df_filtrado['mes'] = pd.to_datetime(df_filtrado['Fecha']).dt.month
    df_filtrado['dia'] = pd.to_datetime(df_filtrado['Fecha']).dt.day

    # ✅ PASO 4: Determinar los 3 meses a analizar (mismo criterio que función original)
    meses_analizar = []
    for i in range(3):
        fecha_mes = hoy.replace(day=1) - relativedelta(months=i)
        meses_analizar.append((fecha_mes.year, fecha_mes.month))

    # ✅ PASO 5: Filtrar solo los días relevantes (hasta el día actual en cada mes)
    # Igual que la lógica original: ultimo_dia = min(dia_actual, dias_en_mes)
    df_filtrado['max_dia_mes'] = df_filtrado.apply(
        lambda row: min(dia_actual, calendar.monthrange(row['año'], row['mes'])[1]),
        axis=1
    )
    df_filtrado = df_filtrado[df_filtrado['dia'] <= df_filtrado['max_dia_mes']]

    # ✅ PASO 6: Agrupar por SKU, Año, Mes (métricas agregadas por SKU×Mes)
    tiempo_agrupacion_inicio = time.time()
    ventas_por_sku_mes = df_filtrado.groupby(['sku', 'año', 'mes']).agg({
        'Total': 'sum',
        'Costo de venta': 'sum',
        'Comision por venta': 'sum',
        'gastos_destino': 'sum',
    ...
    ventas_por_sku_mes.columns = ['sku', 'año', 'mes', 'Ventas_Reales', 'Costo_Venta',
                                   'Comision_Venta', 'Gastos_Destino', 'Ultima_Milla', 'Num_Transacciones']

    # Calcular campos derivados (vectorizado)
    ventas_por_sku_mes['Gastos_Directos'] = (
        ventas_por_sku_mes['Comision_Venta'] +
        ventas_por_sku_mes['Gastos_Destino'] +
        ventas_por_sku_mes['Ultima_Milla']
    )
    ventas_por_sku_mes['Ingreso_Real'] = (
        ventas_por_sku_mes['Ventas_Reales'] -
        ventas_por_sku_mes['Costo_Venta'] -
        ventas_por_sku_mes['Gastos_Directos']
    )

    # Calcular porcentajes (vectorizado)
    ventas_por_sku_mes['Costo_Venta_Porcentaje'] = (
        ventas_por_sku_mes['Costo_Venta'] / ventas_por_sku_mes['Ventas_Reales'] * 100
    ).fillna(0)
    ventas_por_sku_mes['Gastos_Directos_Porcentaje'] = (
        ventas_por_sku_mes['Gastos_Directos'] / ventas_por_sku_mes['Ventas_Reales'] * 100
    ).fillna(0)
    ventas_por_sku_mes['Ingreso_Real_Porcentaje'] = (
        ventas_por_sku_mes['Ingreso_Real'] / ventas_por_sku_mes['Ventas_Reales'] * 100
    ).fillna(0)
    ventas_por_sku_mes['ROI_Porcentaje'] = (
        ventas_por_sku_mes['Ingreso_Real'] / ventas_por_sku_mes['Costo_Venta'] * 100
    ).fillna(0)

    # Calcular promedios por transacción (vectorizado)
    ventas_por_sku_mes['Ventas_Reales_Promedio'] = (
        ventas_por_sku_mes['Ventas_Reales'] / ventas_por_sku_mes['Num_Transacciones']
    ).fillna(0)
    ventas_por_sku_mes['Costo_Venta_Promedio'] = (
        ventas_por_sku_mes['Costo_Venta'] / ventas_por_sku_mes['Num_Transacciones']
    ).fillna(0)
    ventas_por_sku_mes['Gastos_Directos_Promedio'] = (
        ventas_por_sku_mes['Gastos_Directos'] / ventas_por_sku_mes['Num_Transacciones']
    ).fillna(0)
    ventas_por_sku_mes['Ingreso_Real_Promedio'] = (
        ventas_por_sku_mes['Ingreso_Real'] / ventas_por_sku_mes['Num_Transacciones']
    ).fillna(0)

    tiempo_agrupacion_fin = time.time()
    print(f"   ⏱️  Agrupación SKU×Mes: {tiempo_agrupacion_fin - tiempo_agrupacion_inicio:.3f} segundos")

    # ✅ PASO 7: Agrupar por SKU, Año, Mes, Canal (desglose por canal dentro de cada mes)
    tiempo_canales_inicio = time.time()
    ventas_por_sku_mes_canal = df_filtrado.groupby(['sku', 'año', 'mes', 'Channel']).agg({
        'Total': 'sum',
        'Costo de venta': 'sum',
        'Comision por venta': 'sum',
        'gastos_destino': 'sum',
    ...
    ventas_por_sku_mes_canal.columns = ['sku', 'año', 'mes', 'Canal', 'Ventas_Reales', 'Costo_Venta',
                                        'Comision_Venta', 'Gastos_Destino', 'Ultima_Milla', 'Num_Transacciones']

    # Calcular campos derivados para canales (vectorizado)
    ventas_por_sku_mes_canal['Gastos_Directos'] = (
        ventas_por_sku_mes_canal['Comision_Venta'] +
        ventas_por_sku_mes_canal['Gastos_Destino'] +
        ventas_por_sku_mes_canal['Ultima_Milla']
    )
    ventas_por_sku_mes_canal['Ingreso_Real'] = (
        ventas_por_sku_mes_canal['Ventas_Reales'] -
        ventas_por_sku_mes_canal['Costo_Venta'] -
        ventas_por_sku_mes_canal['Gastos_Directos']
    )

    # Calcular porcentajes para canales (vectorizado)
    ventas_por_sku_mes_canal['Costo_Venta_Porcentaje'] = (
        ventas_por_sku_mes_canal['Costo_Venta'] / ventas_por_sku_mes_canal['Ventas_Reales'] * 100
    ).fillna(0)
    ventas_por_sku_mes_canal['Gastos_Directos_Porcentaje'] = (
        ventas_por_sku_mes_canal['Gastos_Directos'] / ventas_por_sku_mes_canal['Ventas_Reales'] * 100
    ).fillna(0)
    ventas_por_sku_mes_canal['Ingreso_Real_Porcentaje'] = (
        ventas_por_sku_mes_canal['Ingreso_Real'] / ventas_por_sku_mes_canal['Ventas_Reales'] * 100
    ).fillna(0)
    ventas_por_sku_mes_canal['ROI_Porcentaje'] = (
        ventas_por_sku_mes_canal['Ingreso_Real'] / ventas_por_sku_mes_canal['Costo_Venta'] * 100
    ).fillna(0)

    # Calcular promedios para canales (vectorizado)
    ventas_por_sku_mes_canal['Ventas_Reales_Promedio'] = (
        ventas_por_sku_mes_canal['Ventas_Reales'] / ventas_por_sku_mes_canal['Num_Transacciones']
    ).fillna(0)
    ventas_por_sku_mes_canal['Costo_Venta_Promedio'] = (
        ventas_por_sku_mes_canal['Costo_Venta'] / ventas_por_sku_mes_canal['Num_Transacciones']
    ).fillna(0)
    ventas_por_sku_mes_canal['Gastos_Directos_Promedio'] = (
        ventas_por_sku_mes_canal['Gastos_Directos'] / ventas_por_sku_mes_canal['Num_Transacciones']
    ).fillna(0)
    ventas_por_sku_mes_canal['Ingreso_Real_Promedio'] = (
        ventas_por_sku_mes_canal['Ingreso_Real'] / ventas_por_sku_mes_canal['Num_Transacciones']
    ).fillna(0)

    tiempo_canales_fin = time.time()
    print(f"   ⏱️  Agrupación SKU×Mes×Canal: {tiempo_canales_fin - tiempo_canales_inicio:.3f} segundos")

    # ✅ PASO 8: Construir estructura de salida indexada por SKU
    tiempo_construccion_inicio = time.time()
    resultado = {}

    for sku in skus_list:
        # Filtrar datos de este SKU
        datos_sku = ventas_por_sku_mes[ventas_por_sku_mes['sku'] == sku]
        datos_sku_canales = ventas_por_sku_mes_canal[ventas_por_sku_mes_canal['sku'] == sku]

        desglose_meses = []

        for año_mes, mes_num in meses_analizar:
            # Filtrar datos del mes específico
            datos_mes = datos_sku[(datos_sku['año'] == año_mes) & (datos_sku['mes'] == mes_num)]

            if datos_mes.empty:
                continue  # No hay datos para este mes

            # Extraer datos del mes (solo debería haber 1 fila por SKU×Mes)
            row = datos_mes.iloc[0]

            # Determinar el último día incluido en el período
            ultimo_dia = min(dia_actual, calendar.monthrange(año_mes, mes_num)[1])
            fecha_mes = datetime(año_mes, mes_num, 1)

            # Construir desglose por canal para este mes
            desglose_canales_mes = []
            datos_canales_mes = datos_sku_canales[
                (datos_sku_canales['año'] == año_mes) &
                (datos_sku_canales['mes'] == mes_num)
            ]

            for _, canal_row in datos_canales_mes.iterrows():
                canal_data = {
            ...
            # Construir datos del mes (MISMA estructura que función original)
            mes_data = {
                'mes': int(mes_num),
                'año': int(año_mes),
        ...
        resultado[sku] = desglose_meses

    tiempo_construccion_fin = time.time()
    print(f"   ⏱️  Construcción estructura salida: {tiempo_construccion_fin - tiempo_construccion_inicio:.3f} segundos")

    tiempo_total = time.time() - tiempo_inicio
    print(f"✅ [OPTIMIZACIÓN] Desglose temporal completado en {tiempo_total:.3f} segundos")
    print(f"   📊 {len(resultado)} SKUs procesados")
    print(f"   🚀 Ganancia estimada vs método anterior: ~{len(skus_list) * 0.3:.1f} segundos")
...
def calcular_desglose_temporal_sku(df_completo, sku):
    """
    Calcula el desglose temporal de los últimos 3 meses para un SKU específico

    ⚠️ NOTA: Esta función se mantiene para compatibilidad, pero se recomienda usar
    calcular_desglose_temporal_todos_skus_optimizado() para procesar múltiples SKUs.
    """
    from datetime import datetime
    from dateutil.relativedelta import relativedelta
    import calendar
    ...
    for fecha_mes in meses_analizar:
        # Determinar el último día a incluir
        ...
        # Filtrar datos del SKU para este mes específico
        ...
        if not ventas_sku_mes.empty:
            ...
            if not ventas_sku_mes.empty:
                ...
                for _, canal_row in ventas_por_canal_mes.iterrows():
                    ...
                    # ✅ NUEVO: Calcular ROI para canal por mes
            ...
            # ✅ NUEVO: Calcular ROI porcentaje para el mes
...
# FUNCIONES EXISTENTES DE ANÁLISIS
...
# Función para obtener SKUs específicos
